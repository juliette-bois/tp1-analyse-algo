    ...
    while useCount < len(results):
        ...
        if child is None:
            ...
            if child[0] == current:
                ...
                current = child[0]
    tree.computeMinCouplage(graph)

    graphToEul = Graph(graph.V)
    graphToEul.graph = []
    for i, node in tree.nodes:
        for child, weight in node.children:
            if graphToEul.get_edge(i, child.number) is None:
                graphToEul.add_edge(i, child.number, weight)
    eulerResult = graphToEul.get_eulerian_cycle()
    cycle = []
    for i in eulerResult:
        if not (i in cycle):
            cycle.append(i)
